 ...
 * \param hasPhysics `bool` – Is this collision box solid? `true` if so, `false` otherwise. Default: `false`
 * \param fixed `bool` – Is the object immovable by force? Default: `false`
 ...
 * \param collisionMax `int` – Size of collisions pointer array (required for cleanup)
 * \param collisions `TW_Entity**` – array of pointers which point to entities that have been collided with
 */
typedef struct TW_Collision {
    ...
    bool hasPhysics;                        // Is this collision box solid? `true` if so, `false` otherwise.  Default: `false`
    bool fixed;                             // Is the object immovable by force?  Default: `false`
 ...
 * 
 * \param x `int` – X position of collision object relative to the entity transform.
